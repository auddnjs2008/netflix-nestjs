...
describe('MovieController (e2e)', () => {
  ...
  beforeAll(async () => {
    ...
    const movieUserLikeRepository = dataSource.getRepository(MovieUserLike);
     ...
     users = [1,2].map((x) => userRepository.create({id:x,email:`${x}@test.com`,password:'123123'}));
     await userRepository.save(users);
    
     directors = [1,2].map(x=> directorRepository.create({id:x,dob:new Date('1996-01-29'),nationality:'South Korea',name:`Directonr Name ${x}`}));
  ...
  describe('[GET /movie/:id]',() => {
    ...
    it('should throw 404 if movie does not exist', async () => {
      const movieId = 99999;
      const {body,statusCode} = await request(app.getHttpServer())
      .get(`/movie/${movieId}`)
      .set('authorization', `Bearer ${token}`);
  ...
  describe('DELETE [/movie/:id]',() => {
    ...
    it('should throw 404 if moive does not exist', async () => {
      const movieId = 99999;
      const {body,statusCode} = await request(app.getHttpServer())
      .patch(`/movie/${movieId}`)
      .set('authorization', `Bearer ${token}`);
  ...
  describe('POST [/mvoei/:id/like', () => {
    ...
    it('should cancel like movie', async () => {
      const movieId = movies[1].id;
      const {body,statusCode} = await request(app.getHttpServer())
      .post(`/movie/${movieId}/like`)
      .set('authorization', `Bearer ${token}`);
  ...
  describe('POST [/mvoei/:id/dislike', () => {
    it('should like movie', async () => {
      const movieId = movies[1].id;
      const {body,statusCode} = await request(app.getHttpServer())
    ...
    it('should cancel dislike movie', async () => {
      const movieId = movies[1].id;
      const {body,statusCode} = await request(app.getHttpServer())
      .post(`/movie/${movieId}/dislike`)
      .set('authorization', `Bearer ${token}`);
