...
describe('AuthController', () => {
  ...
  describe('registerUser', () => {
     it('should register a user', async () => {
        const token = 'Basic asdfasdfasdf';
  ...
  describe('loginUser' , () => {
    it('should loing a user', async () => {
      const token = 'Basic asdfasdfadsf';
      const result = {
  ...
  describe('blockToken', () => {
     it('should bloack a token', async () => {
  ...
  describe('rotateAccessToken', () => {
     it('should rotate access Token', async () => {
      const accesToken = 'mocked.accessToken';
      jest.spyOn(authService,'issueToken').mockResolvedValue(accesToken);
  ...
  describe('login/passport' , () => {
    it('should login user passport', async () => {
      const user = {
        id:1,
        role:Role.user
      };
      const req = {user};
      const accessToken = 'mocked.accessToken';
      const refreshToken = 'mocked.refreshToken';

      jest.spyOn(authService,'issueToken').mockResolvedValueOnce(refreshToken).mockResolvedValueOnce(accessToken);
